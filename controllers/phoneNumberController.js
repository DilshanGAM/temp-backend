...
export const getPhoneNumberById = async (req, res) => {
  ...
  console.log('Fetching phone number with ID:', id);
...
export const updatePhoneNumber = async (req, res) => {
  const { id } = req.params;
  const { phoneNumber, countryCode, numberWithoutCountryCode, country } =
    req.body;
  const email = req.user.email;
...
export const deletePhoneNumber = async (req, res) => {
  ...
  try {
    const deletedPhoneNumber = await PhoneNumber.findOneAndDelete({
      _id: id,
      email,
    });
